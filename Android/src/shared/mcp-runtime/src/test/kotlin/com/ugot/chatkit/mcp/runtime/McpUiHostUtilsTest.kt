...
class McpUiHostUtilsTest {
  ...
  fun injectHostBridge_marksHostAsMcpAppsAndExposesWidgetApis() {
    ...
    val injected = McpUiHostUtils.injectHostBridge(html)

    assertTrue(injected.contains("__hostType: 'mcp-apps'"))
    assertTrue(injected.contains("bridge.getToolName()"))
    assertTrue(injected.contains("bridge.getToolInput()"))
    assertTrue(injected.contains("bridge.getToolOutput()"))
    assertTrue(injected.contains("bridge.getWidgetState()"))
    ...
    assertTrue(injected.contains("name=\"viewport\""))
    assertTrue(injected.contains("overflow-x: hidden"))
  }
}
